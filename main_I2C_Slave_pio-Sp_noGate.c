 ...
 * A Companion App, RPi-Text-Reader-Sp, running on a RPi or RPi Zero reads Text
 * and sends the text a char at a time to the Front of a Ring Buffer on
 * the Pico using I2S functions.  The RPi is the I2C Master
 * and the Pico is the I2C Slave. A Queue takes the Tail of the Ring
 * Buffer and sends the data to the character CW send function.
 * At the beginning of the Text File is an instruction for setting 
 * the Code WPM speed desired.  The I2C function sends this Code Speed
 * instruction to the I2C Registers where it is picked up by the Timer
 * function setting the CW bit rate.
 * 
 * cd ~/FreeRTOS-CW-I2C/build/App-CW-pio-I2C-Slave
    ...
    sudo scp Code-Groups-2.txt pi2@10.0.0.197:./Play
    
    ./RPi-Text-Reader-Sp Nutcracker.txt
    ./RPi-Text-Reader-Sp Gardeners_Grandmother.txt
    ./RPi-Text-Reader-Sp Code-Groups.tx 
    ./RPi-Text-Reader-Sp Code-Groups-2.txt
...
//#define PIO_TONE_GPIO   7         // pin  10
#define DOT_PERIOD      120
#define TONE_FREQ       500
#define TONE_GPIO       17        // pin 22
#define CW_GPIO         21        // pin 27 
#define CW_LED_GPIO     0         // pin  1
#define I2C_LED0_GPIO   1         // pin  2
#define I2C0_SDA_GPIO   12        // pin  16
#define I2C0_SCL_GPIO   13        // pin  17
#define I2C0_IRQ        23      
...
void i2c_task(void* unused_arg) {
 ...
 // ************* Code Speed Adjustment from Text ********************************       
            xTimerChangePeriod( cwd_timer,context.mem[SPD_INDEX],0 );
 // ******************************************************************************       
 ...
 *        An extra not printed space character can be added between each char to 
 *        slow the CW transmission without slowing the actual speed of the CW sent.
 * 
 */
...
//                send_CW(0x20);  //  extra Space char after each Char sent to slow
                send_CW(0x10);  //  even longer Space char after each Char sent to slow
            }
           //************* select fast CW or slow CW ****************************************  
       } // end of if(...
    }  // end of while(true)
 ...
 * The CW signal is sent to one input of a NOR gate.  The second NOR gate input is a signal
 * generated by a pio function outputting a continuous 500 Hz square wave.  The CW signal switches 
 * the square wave output of the NOR gate ON and OFF to generate the Audio CW.
 */
...
// *********************************************************************************************     
        ...
        else if (this_char == 0x20) { // look for normal SPACE char
    ...
    while (morse_out != 1) {              // send Morse bits              
        ...
        if(bit_out){
            pio_sm_set_enabled(pio, 0, 1);
        }
        else{
            pio_sm_set_enabled(pio, 0, 0);
        }
        gpio_put(CW_LED_GPIO, !bit_out);         //  Dots need LOW to turn ON   D11_P15
...
void configure_gpio(void) {
    ...
    // Configure TONE_GPIO for 500 Hz out to audio gate
    gpio_init(TONE_GPIO);
...
//    context.mem[SPD_INDEX] = 120;
    ...
    blink_pin_forever(pio, 0, offset, TONE_GPIO, TONE_FREQ);  // gpio 17-pin 22, approx 500 Hz
    // PIO GPIO may be almost any GPIO, where TONE GPIO for CLK output needs to be GPIO 21 pin 27
//    blink_pin_forever(pio, 0, offset, PIO_TONE_GPIO, TONE_FREQ);  // gpio 7-pin 10, approx 500 Hz
    ...
    dot_period = context.mem[SPD_INDEX];
 
// Timer creates dot length with ADC
